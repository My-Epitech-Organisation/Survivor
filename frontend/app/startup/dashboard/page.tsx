...
export default function StartupDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchProjectViewsOverTime = async () => {
      const projectViewsOverTimeResponse = await authenticatedFetch(
        `/projectViews/${user?.id}`
      );
      if (!projectViewsOverTimeResponse.ok) {
        throw new Error("Failed to fetch project views");
      }
      const projectViewsOverTime: ProjectViewsOverTime[] =
        await projectViewsOverTimeResponse.json();
    ...
    const fetchProjectEngagement = async () => {
      ...
      const projectEngagement: ProjectEngagement =
