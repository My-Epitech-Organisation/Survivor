...
const Protectedroutes: RouteRoles[] = [
  ...
  {
    route: "/profile",
    rolesAuth: ["investor", "admin", "user", "founder"],
  },
];
...
export async function middleware(request: NextRequest) {
  ...
  const matchedRoute = Protectedroutes.find((pRoute) =>
    path.startsWith(pRoute.route)
  );
...
export const config = {
  matcher: ["/projects/:path*", "/admin/:path*", "/startup/:path*", "/investor/:path*", "/profile"],
};
