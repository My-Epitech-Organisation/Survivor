                ...
                <div className="flex-shrink-0 w-full lg:w-[50%] xl:w-[60%]">
                    ...
                    <div className="grid grid-cols-7 gap-1 mb-2">
                        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                            <div key={day} className="p-1 sm:p-2 text-center font-medium text-app-text-secondary text-xs sm:text-sm">
                    ...
                    <div className="grid grid-cols-7 gap-1">
                        {calendarDays.map((day, index) => {
                            if (day === null) {
                                return <div key={`empty-${index}`} className="p-1 sm:p-2 h-16 sm:h-20 lg:h-24"></div>;
                            }
                            ...
                            return (
                                <div
                                    key={`day-${index}`}
                                    className={`p-1 sm:p-2 h-16 sm:h-20 lg:h-24 border border-app-border-light rounded transition-colors hover:bg-app-surface-hover ${isTodayDay ? 'bg-app-blue-light border-app-blue-primary' : 'bg-app-surface'
                                        }`}
                                >
                                    <div className={`font-medium text-xs sm:text-sm mb-1 ${isTodayDay ? 'text-app-blue-primary' : 'text-app-text-primary'
                                        }`}>
                                        {day}
                                    </div>
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                                                <h4 className="font-medium text-app-text-primary text-sm sm:text-base">{event.name}</h4>
                                                <Badge className={`${getEventTypeColor(event.event_type)} text-xs self-start sm:self-auto`}>
                                                    {event.event_type}
                                                </Badge>
                                            </div>
                                            <p className="text-xs sm:text-sm text-app-text-secondary mb-2">{event.description}</p>
                                            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 text-xs text-app-text-muted">
                                                <span className="flex items-center gap-1">📅 {event.parsedDate.toLocaleDateString()}</span>
                                                <span className="flex items-center gap-1">📍 {event.location}</span>
                                                <span className="flex items-center gap-1">👥 {event.target_audience}</span>
                                            </div>
                                        </div>
                                    </div>
                                </Card>
