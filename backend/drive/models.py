...
class DriveFile(models.Model):
    ...
    name = models.CharField(max_length=255, help_text="Name of the file with extension")
    file = models.FileField(upload_to="drive_files/%Y/%m/%d/", help_text="The actual file content")
    ...
    description = models.TextField(null=True, blank=True, help_text="Optional description of the file")
    is_archived = models.BooleanField(default=False, help_text="Whether the file is archived (hidden but not deleted)")
...
class DriveShare(models.Model):
    ...
    access_token = models.CharField(max_length=64, unique=True, help_text="Unique token for accessing the shared item")
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="When the share expires (null for no expiration)"
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=models.Q(file__isnull=False) | models.Q(folder__isnull=False), name="share_either_file_or_folder"
            ),
            models.CheckConstraint(
...
class DriveActivity(models.Model):
    ...
    action = models.CharField(max_length=20, choices=ACTION_CHOICES, help_text="The type of action performed")
    timestamp = models.DateTimeField(auto_now_add=True, help_text="When the action was performed")
    details = models.JSONField(null=True, blank=True, help_text="Additional details about the action (JSON format)")
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address of the user who performed the action"
    )
