...
class DriveFolderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DriveFolder
        fields = [
            'id', 'startup', 'name', 'parent', 'created_at', 'created_by',
            'subfolders_count', 'files_count', 'path'
        ]
...
class DriveFileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DriveFile
        fields = [
            'id', 'startup', 'folder', 'name', 'file', 'size', 'human_size',
            'file_type', 'uploaded_by', 'uploaded_by_name', 'uploaded_at',
            'last_modified', 'description', 'is_archived', 'path', 'download_url'
        ]
        read_only_fields = [
            'uploaded_at', 'last_modified', 'size', 'human_size',
            'file_type', 'path', 'download_url'
        ]
...
class DriveFileListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DriveFile
        fields = [
            'id', 'name', 'folder', 'file_type', 'human_size',
            'uploaded_at', 'uploaded_by_name', 'is_archived'
        ]
...
class DriveShareSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DriveShare
        fields = [
            'id', 'file', 'folder', 'shared_item_name', 'shared_item_type',
            'shared_by', 'shared_by_name', 'shared_at', 'access_token',
            'expires_at', 'is_active', 'is_expired'
        ]
        read_only_fields = [
            'shared_at', 'access_token', 'is_expired',
            'shared_item_name', 'shared_item_type', 'shared_by_name'
        ]
...
class DriveActivitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DriveActivity
        fields = [
            'id', 'startup', 'user', 'user_name', 'file', 'folder',
            'item_name', 'item_type', 'action', 'timestamp', 'details'
        ]
        read_only_fields = [
            'startup', 'user', 'user_name', 'file', 'folder',
            'item_name', 'item_type', 'action', 'timestamp', 'details'
        ]
